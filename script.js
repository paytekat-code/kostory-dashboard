...
window.showCheckinList = async function() {
  ...
  Object.keys(all).forEach(kost => {
    Object.keys(all[kost] || {}).forEach(room => {
      ...
      if (d && d.tanggalMasuk) {
        ...
        const coDate = new Date(d.tanggalMasuk);
        if (coDate.getMonth() === thisMonth && coDate.getFullYear() === thisYear) {
          bulanIni.push(item);
        } else {
          sebelumnya.push(item);
  ...
  sebelumnya.sort((a,b) => new Date(b.tanggalMasuk) - new Date(a.tanggalMasuk));

  const renderItem = (d, i) => `
    <div class="checkout-item" onclick="openModal('${d.kost}','${d.room}')">
      <strong>${i+1}. ${d.room} | ${d.nama}</strong><br>
      <small>${d.durasi} | ${formatDate(d.tanggalMasuk)} | Token Awal: ${d.tokenAwal} | ${d.status}</small>
    </div>
  `;

  document.getElementById("checkinBulanIni").innerHTML = bulanIni.map(renderItem).join("") || "<p style='text-align:center;color:#666;padding:30px'>Belum ada check-in</p>";
  document.getElementById("checkinBulanLalu").innerHTML = sebelumnya.map(renderItem).join("") || "<p style='text-align:center;color:#666;padding:30px'>Tidak ada</p>";
};

// TOMBOL LAPORAN KE WA
window.laporCheckinWA = async function() {
  ...
  if (bulanIni.length === 0) {
    ...
    bulanIni.forEach((d, i) => {
      pesan += `${i+1}. ${d.room} | ${d.nama} | ${formatDate(d.tanggalMasuk)} | ${d.durasi} | ${d.tokenAwal} | ${d.status}\n`;
    });
    pesan += "\n";
  }

  pesan += "Bulan lalu : " + sebelumnya.length + " Orang\n\n";
  if (sebelumnya.length === 0) {
    pesan += "Tidak ada\n";
  } else {
    sebelumnya.forEach((d, i) => {
